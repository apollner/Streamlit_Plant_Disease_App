...
st.write('### The PlantVillage dataset consist of 38 categories of plants/diseases')
...
if url:
  ...
  if classify:
        st.write("")
        st.write("Classifying...")
        new_img = tf.keras.preprocessing.image.load_img(bytes_data, target_size=(224, 224))
        img = tf.keras.preprocessing.image.img_to_array(new_img)
        img = np.expand_dims(img, axis=0)
        prediction = mobilenet_v3.predict(img)
        d = prediction.flatten()
